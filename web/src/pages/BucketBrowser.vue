<template>
  <div class="space-y-6">
    <h1 class="text-xl font-semibold">Bucket 浏览</h1>

    <div class="flex items-center gap-3">
      <label class="text-sm text-gray-600">选择 Bucket：</label>
      <select
      ...
      <button class="ml-2 px-3 py-1 border rounded" @click="refreshBuckets" :disabled="loadingBuckets">
    ...
    <div v-if="selectedBucket" class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        <button class="px-3 py-1 border rounded" @click="loadFiles" :disabled="loadingFiles">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr v-for="f in files" :key="f.ID" class="border-t">
              ...
              <td class="px-3 py-2">{{ f.MimeType || '-' }}</td>
